...
"$PYTHON_BIN" tools/filter_depth_h5.py "$ROOT" --find-meta true --min-distance 0.01 --max-distance 3.0 \
    --spatial-alpha 0.5 --spatial-delta 20 --spatial-magnitude 2 \
    --temporal-alpha 0.8 --temporal-delta 50 
...
"$PYTHON_BIN" tools/estimate_camera_poses_from_apriltag.py "$ROOT" --tag-map "$TAG_MAP" --pnp-method ippe --no-ransac --output-name camera_poses_apriltag.json --threads 0
...
"$PYTHON_BIN" tools/run_sam2_apriltag_detect_propagate.py "$ROOT" --cuda-devices 0 --chunk-size 1500 --async-loading-frames --diffueraser-chunk-size 300 --prompt-stride 30
