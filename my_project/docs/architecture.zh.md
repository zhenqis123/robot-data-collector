...
4. **展示层**：`Preview` 将 `cv::Mat` 转换成 `QImage`/`QPixmap`，根据相机类型动态构建预览面板，RealSense 同时展示 RGB+深度；`MainWindow` 负责控制面板、状态提示以及“采集名称/被试信息”输入，存储根目录由 `resources/config.json` 的 `captures_path` 指定，并提供打开/关闭摄像头、开始/停止/暂停/继续采集等操作。
